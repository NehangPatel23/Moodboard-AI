...
- **Auth callback** — `src/app/auth/callback/route.ts` exchanges Supabase auth codes for a session (`exchangeCodeForSession`) and handles `token_hash` recovery links; redirects to `?next=` (password reset lands on `/sign-in?mode=update-password`). Requires `/auth/callback` in Supabase redirect URLs.
...
2. Run migrations in [`supabase/migrations/`](../supabase/migrations/) through **`032`** in the SQL Editor (or follow [SUPABASE_SETUP.md](SUPABASE_SETUP.md) / [DEPLOY.md](DEPLOY.md) for the full ordered list).
